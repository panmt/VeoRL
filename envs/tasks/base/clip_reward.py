...
class ClipReward(ABC):
    ...
    def _get_reward_from_logits(
        ...
        '''
        首先，它从 probs 中取出第一个元素，即模型认为当前观测最可能属于的类别的概率，并将其转换为 Python 的浮点数（通过调用 .item() 方法）。
        然后，它从这个概率中减去 1 / logits.shape[0]，也就是类别数量的倒数，这可能是为了引入一个基线值或者惩罚项。
        最后，它使用 max 函数确保最终的奖励值不会小于 0，因为负的奖励值可能没有实际意义或者不符合特定的奖励策略。
        '''
    ...
    def _get_image_feats(
        ...
        '''
        从当前帧（curr_frame）和过去的帧（past_frames）中提取特征。
        '''
