...
class MetaWorld:
    ...
    def step(self, action):
        ...
        obs['image_ori'] = obs['image']
        img_resized = cv2.resize(obs['image'], (64, 64), interpolation=cv2.INTER_AREA)
        return {'image': img_resized, 'image_ori': obs['image_ori'], 'state': obs['state']}, obs['reward'], False, info
    ...
    def reset(self):
        ...
        obs['image_ori'] = obs['image']
        img_resized = cv2.resize(obs['image'], (64, 64), interpolation=cv2.INTER_AREA)
        return {'image': img_resized, 'image_ori': obs['image_ori'], 'state': obs['state']}
  ...
  def step(self, action):
    ...
    if done:
      ...
      if self._mode == 'eval':
        ...
        self._mean_episode_success += (float(np.sum(episode['success']) >= 1.0)) * (1/self.eval_num)
      ...
      if (self._episode_count == self.eval_num) and (self._mode == 'eval'):
        ...
        print(self._mean_episode_success)
        self.log_success = self._mean_episode_success
        self._logger.write()
        self._mean_episode_score = 0
        self._mean_episode_success = 0
        self._episode_count = 0
